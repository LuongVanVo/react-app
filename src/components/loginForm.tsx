import { cn } from "@/lib/utils";
import type React from "react";
import type { JSX } from "react";
import { FaGithub } from "react-icons/fa";

import { Button } from "@/components/ui/button";
import { Label } from "@/components/ui/label";
import { Input } from "@/components/ui/input";

function LoginForm({
    className,
    ...props
}: {
    className?: string;
} & React.ComponentProps<"form">): JSX.Element {
    return (
        <form className={cn(className)} {...props}>
            {/* Tiêu đề và giới thiệu */}
            <div className="mb-6 space-y-2 text-center">
                <h1 className="text-2xl font-bold text-white">Login to your account</h1>
                <p className="text-muted-foreground">Enter your email below to login to your account</p>
            </div>

            {/* Nhập email */}
            <div className="mb-4 space-y-2">
                <Label htmlFor="email" className="text-white">Email</Label>
                <Input id="email" type="email" placeholder="m@example.com" className="bg-[#1E1E1E] text-white border-gray-600" required />
            </div>

            {/* Nhập mật khẩu và Forgot Password */}
            <div className="mb-6 space-y-2">
                <div className="flex justify-between">
                    <Label htmlFor="password" className="text-white ">Password</Label>
                    <a href="#" className="text-white hover:underline text-sm">Forgot your password?</a>
                </div>
                <Input id="password" type="password" className="bg-[#1E1E1E] text-white border-gray-600" required />
            </div>

            {/* Nút Đăng nhập */}
            <Button className="bg-white text-black w-full hover:bg-white/90 mb-4">
                Login
            </Button>

            {/* Or continue with Google*/}
            <div className="flex items-center gap-2 justify-center">
                <div className="flex-1 h-px bg-gray-600"></div>
                <span className="text-muted-foreground">Or continue with</span>
                <div className="flex-1 h-px bg-gray-600"></div>
            </div>

            {/* login with github */}
            <Button className="bg-[#1E1E1E] text-white w-full border border-gray-600 hover:bg-[#2A2A2A] flex items-center justify-center gap-2 mt-4">
                <FaGithub />
                Login with GitHub
            </Button>

            {/* Chưa có tài khoản? Đăng ký */}
            <div className="text-center text-white mt-4">
                <span className="text-white text-sm">
                    <p>Don't have an account? <a href="#" className="underline">Sign up</a></p>
                </span>
            </div>
        </form>
    );
}

export { LoginForm };